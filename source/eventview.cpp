...
void EventView::displayCalendar(const CalendarVisualData& data)
{
    if (!m_scene) return;
    ...
    QTimer::singleShot(10, this, &EventView::fitToView);
}

void EventView::setZoomLevel(qreal level)
{
...
void EventView::setInteractive(bool interactive)
{
    setDragMode(interactive ? QGraphicsView::RubberBandDrag : QGraphicsView::NoDrag);
    setInteractive(interactive);
}

void EventView::setAntialiasingEnabled(bool enabled)
{
...
void EventView::resizeEvent(QResizeEvent *event)
{
    ...
    if (m_scene && !m_scene->items().isEmpty()) {
        QTimer::singleShot(0, this, &EventView::fitToView);
    }
}
...
void EventView::handleSceneItemClicked(CalendarItem* item)
{
...
void EventView::calculateOptimalZoom()
{
    if (!m_scene || m_scene->items().isEmpty()) return;
